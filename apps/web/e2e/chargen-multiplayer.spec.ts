...
test.describe('Multiplayer Chargen', () => {
  ...
  test.describe('Multiplayer Chargen Session', () => {
    ...
    test('chargen page renders participant panel', async ({ page }) => {
      ...
      await expect(
        page
          .locator('[data-testid="participant-panel"]')
          .or(page.getByText(/session participants/i))
          .or(page.getByText(/loading session/i)),
  ...
  test.describe('Connection Request Approval Flow', () => {
    test('entity pool panel renders', async ({ page }) => {
      ...
      await expect(
        page
          .locator('[data-testid="entity-pool"]')
          .or(page.getByText(/spawned entities/i).or(page.getByText(/entity pool/i))),
      ).toBeVisible({ timeout: 10000 });
    });
  });
  ...
  test.describe('Multi-User Sync Tests', () => {
    ...
    test.fixme('entity creation syncs between two users', async ({ browser }) => {
      ...
      await page1
        .locator('button:has-text("Create Entity")')
        .or(page1.locator('button:has-text("Spawn Entity")'))
        .click();
    ...
    test.fixme('participant join request syncs to GM', async ({ browser }) => {
      ...
      await pagePlayer
        ...
        .or(pagePlayer.locator('button:has-text("Join")'))
        .click();
  ...
  test.describe('Persistence Tests', () => {
    ...
    test.fixme('entities persist after page reload', async ({ page }) => {
      ...
      await expect(page.getByText(/spawned entities/i)).toBeVisible({ timeout: 10000 });
    });
  });
});
