...
- `app/` - Next.js App Router pages
  - `graph/` - Main interactive graph view
  - `chargen/` - Collaborative character creation flow
- `components/`
  - `chargen/` - Collaborative character creation UI
    - `ChargenWizard.tsx`, `GMControlPanel.tsx`, `EntityPoolPanel.tsx`, `EntitySpawnForm.tsx`
    - `CrossCharacterProposalList.tsx`, `CharacterPreview.tsx`, `ServiceRecord.tsx`, `LifepathTimeline.tsx`
    - `steps/` - Multi-step wizard screens
  - `graph/`
    - `GraphCanvas.tsx` - Main React Flow wrapper
    - `CareerNode.tsx` - Career node renderer
    - `Presence.tsx` - Collaborative presence display
  - `tables/`
    - `ReputationTable.tsx` - TanStack Table wrapper
    - `CellHistory.tsx` - Cell change history
...
| `lib/chargen/state.ts`          | Yjs state management for collaborative character creation                               |
| `lib/chargen/hooks.ts`          | React hooks for chargen state                                                           |
| `lib/chargen/types.ts`          | AIProvenance, SessionSettings, and proposal types                                       |
| `lib/chargen/gm-approval.ts`    | Draft visibility and review helpers: `shouldShowDraft`, `isCanonical`, `requiresReview` |
| `lib/chargen/narrative.ts`      | Client-side narrative API                                                               |
| `lib/chargen/entity-spawner.ts` | Entity spawning with provenance                                                         |
...
- Key tests:
  ...
  - `tables.spec.ts`: Cell editing, sorting
  - `sync-latency.spec.ts`: Multi-user consistency
...
- **Services**:
  - `pnpm e2e` starts Hocuspocus through Playwright's `webServer` config and PostgreSQL through global setup.
- **Isolation**:
  - Tests must start with a clean slate.
  - Use `await page.evaluate(() => indexedDB.deleteDatabase('highport-graph'))`.
- **Sync Waiting**:
...
## CHARGEN STATE INVARIANTS

- Yjs `Y.Map` is authoritative for all shared proposals: `crossCharacterLinks`, `lifepathProposals`, and `entityPool`.
- `AIProvenance` follows the lifecycle draft → GM review → accepted, rejected, or edited.
- GM approval modes control draft display: strict hides drafts, moderate shows a badge, and lenient shows drafts.
- `shouldShowDraft()` in `apps/web/lib/chargen/gm-approval.ts` is the display gate.
- `resolveAIDraft()` in `apps/web/lib/chargen/state.ts` handles GM resolution.
- Player edits retain `pendingReviewBy: 'gm'` in strict and moderate modes.
- Cross-character links require dual acceptance, with `acceptedBy` stored as a nested `Y.Map`.
- Entity spawning preserves provenance through `spawnEntity()`.
- Reference: [Chargen state architecture](../../docs/architecture/chargen-state.md)
