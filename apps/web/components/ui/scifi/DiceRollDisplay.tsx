...
export const DiceRollDisplay = memo(function DiceRollDisplay({
  ...
  if (compact) {
    return (
      ...
      >
        {label && <span className="text-gray-400 text-xs uppercase tracking-wider mr-1">{label}</span>}
      ...
      <div className="flex gap-1.5">
        {rolls.map((die, i) => (
          <div 
            ...
            style={{
              background: 'linear-gradient(135deg, var(--nebula-mist-90) 0%, var(--star-metal-90) 100%)',
      ...
      <div className="flex items-center gap-2 font-mono text-sm">
        ...
        {modifier !== 0 && (
          <>
            ...
            {attribute && (
              <span className="text-gray-500 text-xs">({attribute})</span>
            )}
            <span className="text-gray-500">=</span>
          </>
        )}
