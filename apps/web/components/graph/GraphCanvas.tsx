...
function GraphCanvasContent() {
      ...
      edgesMap.forEach((yMap: any) => {
        ...
        loadedEdges.push({
          ...
          style: {
            ...
            strokeDasharray:
              edge.style === 'dashed' ? '5,5' : edge.style === 'dotted' ? '2,2' : undefined,
          },
          markerEnd: {
  ...
  const handleContextAction = useCallback(
    (action: 'edit' | 'delete' | 'lock' | 'unlock' | 'hide' | 'unhide') => {
  ...
  return (
    ...
    >
      ...
      >
        <CosmicBackground
          showStars={!heavyGraph}
          showGrid={!heavyGraph}
          intensity={heavyGraph ? 'low' : 'medium'}
        />
        {/* Vignette overlay */}
        <div
          ...
          style={{
            background:
              'radial-gradient(ellipse at center, transparent 40%, rgba(10, 13, 20, 0.7) 100%)',
          }}
        />
        <Controls />
        ...
        <Panel position="top-right" className="pointer-events-auto" style={{ zIndex: 20 }}>
          <div className="flex gap-4 items-center relative z-10 pointer-events-auto">
            <UserList
              users={[...(currentUser ? [currentUser] : []), ...remoteUsers]}
            ...
            <div className="flex flex-col items-end gap-1">
              ...
              {isGM && (
                <span className="text-xs font-bold text-amber-500 bg-black/80 px-2 py-1 rounded">
                  GM MODE
                </span>
              )}
            </div>
