...
const THEME_CONFIG = {
  cyan: {
    text: { 100: 'text-cyan-100', 200: 'text-cyan-200', 300: 'text-cyan-300', 400: 'text-cyan-400' },
...
export function useSequenceController(steps: number, intervalTime: number | number[], active?: boolean): number { /* ... */ }
...
export function useTransitionController(targetId: string, delay?: number): {
  displayedId: string;
...
/* === TYPOGRAPHY SCALE === */
.title-hero     { font: 900 3rem/1 var(--font-display); letter-spacing: 0.1em; }
.title-section  { font: 700 1.5rem/1.2 var(--font-display); letter-spacing: 0.05em; }
.title-card     { font: 600 1rem/1.3 var(--font-body); }
.body-text      { font: 400 0.875rem/1.5 var(--font-body); }
.data-label     { font: 500 0.75rem/1 var(--font-mono); letter-spacing: 0.15em; text-transform: uppercase; }
.data-value     { font: 700 1.125rem/1 var(--font-mono); }
```
...
<div className={cn(
  ...
  <div className="relative z-10">
    ...
    <div className="flex gap-2">
      {survived && (
        <span className="px-2 py-0.5 bg-warp-emerald/20 border border-warp-emerald/30 
                        rounded text-warp-emerald text-xs font-mono flex items-center gap-1">
          <Shield className="w-3 h-3" /> SURVIVED
        </span>
      )}
      {advanced && (
        <span className="px-2 py-0.5 bg-reactor-amber/20 border border-reactor-amber/30 
...
const CharacterNode = ({ data }) => (
  <div className={cn(
    ...
    {/* Orbiting ring */}
    <div className="absolute inset-[-8px] rounded-full border border-plasma-cyan/20 
...
const EntityNode = ({ data }) => {
  ...
  return (
    <div className={cn(
      ...
      colors.border, `shadow-[0_0_15px_${colors.glow}]`,
...
const EnergyEdge = ({ id, sourceX, sourceY, targetX, targetY, data }) => {
  ...
  return (
    <>
      {/* Outer glow */}
      <path d={path} stroke={data.color} strokeWidth={4} fill="none" 
      ...
      <path d={path} stroke={data.color} strokeWidth={1.5} fill="none"
            strokeDasharray="8 12" className="animate-dash" opacity={0.8} />
...
const DiceRollDisplay = ({ roll, target, modifier, success }) => (
  <div className={cn(
    ...
    <div className="flex items-center gap-2 text-sm font-mono">
      ...
      {modifier !== 0 && (
        <>
          <span className="text-text-muted">{modifier > 0 ? '+' : ''}{modifier}</span>
          <span className="text-text-secondary">=</span>
    ...
    <div className={cn(
      ...
      success ? "bg-warp-emerald/20 text-warp-emerald" : "bg-hull-breach-red/20 text-hull-breach-red"
    )}>
...
*This plan was generated based on comprehensive analysis of the current codebase, research into Mongoose Traveller visual aesthetics, modern sci-fi UI patterns, and reusable components from the present codebase.*
